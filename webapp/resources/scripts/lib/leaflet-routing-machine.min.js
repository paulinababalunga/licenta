(function (factory) {
    ...
    (function () {
        "use strict";
        L.Routing = L.Routing || {};
        L.Routing.Autocomplete = L.Class.extend({options: {timeout: 500, blurTimeout: 100, noResultsMessage: "No results found."}, initialize: function (elem, callback, context, options) {
    ...
    (function () {
        ...
        L.Routing._jsonp = function (url, callback, context, jsonpParam) {
            var callbackId = "_l_routing_machine_" + L.Routing._jsonpCallbackId++, script;
        ...
        L.Routing.OSRM = L.Class.extend({options: {serviceUrl: "//router.project-osrm.org/viaroute", geometryPrecision: 6}, initialize: function (options) {
            ...
            if (response.status !== 0) {
                callback.call(context, {status: response.status, message: response.message});
                return
            }
            var alts = [
                {name: response.route_name.join(", "), coordinates: this._decode(response.route_geometry, this.options.geometryPrecision), instructions: this._convertInstructions(response.route_instructions), summary: this._convertSummary(response.route_summary), waypoints: response.via_points}
            ], i;
            if (response.alternative_geometries) {
                for (i = 0; i < response.alternative_geometries.length; i++) {
                    alts.push({name: response.alternative_names[i].join(", "), coordinates: this._decode(response.alternative_geometries[i], this.options.geometryPrecision), instructions: this._convertInstructions(response.alternative_instructions[i]), summary: this._convertSummary(response.alternative_summaries[i]), waypoints: response.via_points})
                }
            }
            ...
            return this.options.serviceUrl + "?" + "instructions=true&" + locs.join("&") + (this._hints.checksum !== undefined ? "&checksum=" + this._hints.checksum : "")
        }, _locationKey: function (location) {
            return location.lat + "," + location.lng
        }, _saveHintData: function (route, waypoints) {
            var hintData = route.hint_data, loc;
            this._hints = {checksum: hintData.checksum, locations: {}};
            ...
            return{totalDistance: osrmSummary.total_distance, totalTime: osrmSummary.total_time}
        }, _convertInstructions: function (osrmInstructions) {
            var result = [], i, instr, type, driveDir;
            for (i = 0; i < osrmInstructions.length; i++) {
                instr = osrmInstructions[i];
                type = this._drivingDirectionType(instr[0]);
                driveDir = instr[0].split("-");
                if (type) {
                    result.push({type: type, distance: instr[2], time: instr[4], road: instr[1], direction: instr[6], exit: driveDir.length > 1 ? driveDir[1] : undefined, index: instr[3]})
                }
            }
            return result
    ...
    (function () {
        "use strict";
        L.Routing = L.Routing || {};
        L.Routing.Line = L.Class.extend({includes: L.Mixin.Events, options: {styles: [
            {color: "black", opacity: .15, weight: 7},
            {color: "white", opacity: .8, weight: 4},
            {color: "orange", opacity: 1, weight: 2}
        ], addWaypoints: true}, initialize: function (route, options) {
            L.Util.setOptions(this, options);
            this._route = route;
            ...
            var geom = this._route.coordinates, i, pl;
            this._map = map;
            this._layers = [];
            ...
            var wps = this._route.waypoints, indices = [], i;
            ...
            this.fire("linetouched", {afterIndex: afterIndex, latlng: e.latlng})
        }});
    ...
    (function () {
        ...
        L.Routing.Itinerary = L.Control.extend({includes: L.Mixin.Events, options: {units: "metric", pointMarkerStyle: {radius: 5, color: "#03f", fillColor: "white", opacity: 1, fillOpacity: .7}, summaryTemplate: "<h2>{name}</h2><h3>{distance}, {time}</h3>", distanceTemplate: "{value} {unit}", timeTemplate: "{time}", unitNames: {meters: "m", kilometers: "km", yards: "yd", miles: "mi", hours: "h", minutes: "mín", seconds: "s"}, containerClassName: "", alternativeClassName: "", minimizedClassName: "", itineraryClassName: "", roundingSensitivity: 1, show: true}, initialize: function (options) {
            ...
            altDiv.innerHTML = L.Util.template(this.options.summaryTemplate, {name: alt.name, distance: this._formatDistance(alt.summary.totalDistance), time: this._formatTime(alt.summary.totalTime)});
            ...
            var table = L.DomUtil.create("table", this.options.itineraryClassName), body = L.DomUtil.create("tbody", "", table), i, instr, row, td;
            ...
            L.DomEvent.addListener(row, "mouseover", function () {
                marker = L.circleMarker(coordinate, _this.options.pointMarkerStyle).addTo(_this._map)
            });
            ...
            var altElem, j, n, isCurrentSelection, classFn;
            ...
            L.DomEvent.stop(e)
        }, _formatDistance: function (d) {
            var un = this.options.unitNames, v, data;
            if (this.options.units === "imperial") {
                d = d / 1.609344;
                if (d >= 1e3) {
                    data = {value: this._round(d) / 1e3, unit: un.miles}
                } else {
                    data = {value: this._round(d / 1.76), unit: un.yards}
                }
            } else {
                v = this._round(d);
                data = {value: v >= 1e3 ? v / 1e3 : v, unit: v >= 1e3 ? un.kilometers : un.meters}
            }
            return L.Util.template(this.options.distanceTemplate, data)
        }, _round: function (d) {
            var pow10 = Math.pow(10, (Math.floor(d / this.options.roundingSensitivity) + "").length - 1), r = Math.floor(d / pow10), p = r > 5 ? pow10 : pow10 / 2;
            ...
            if (instr.type !== undefined) {
                return L.Util.template(this._getInstructionTemplate(instr, i), L.extend({exit: this._formatOrder(instr.exit), dir: this._dir[instr.direction]}, instr))
            } else {
                return instr.text
            }
        ...
        }, _dir: {N: "north", NE: "northeast", E: "east", SE: "southeast", S: "south", SW: "southwest", W: "west", NW: "northwest"}});
    ...
    (function () {
        "use strict";
        var Waypoint = L.Class.extend({initialize: function (latLng, name) {
            this.latLng = latLng;
            this.name = name
        }});
        L.Routing = L.Routing || {};
        L.Routing.Plan = L.Class.extend({includes: L.Mixin.Events, options: {dragStyles: [
            {color: "black", opacity: .15, weight: 7},
            {color: "white", opacity: .8, weight: 4},
            {color: "orange", opacity: 1, weight: 2, dashArray: "7,12"}
        ], draggableWaypoints: true, addWaypoints: true, maxGeocoderTolerance: 200, autocompleteOptions: {}, geocodersClassName: "", geocoderPlaceholder: function (i, numberWaypoints) {
            ...
            var args = [arguments[0], arguments[1]], i, wp;
            ...
            var container = L.DomUtil.create("div", "leaflet-routing-geocoders " + this.options.geocodersClassName), waypoints = this._waypoints, i, geocoderElem, addWpBtn;
            ...
            var geocoderElem = L.DomUtil.create("input", ""), wp = this._waypoints[i];
            ...
            new L.Routing.Autocomplete(geocoderElem, function (r) {
                ...
                this._updateMarkers();
                this._fireChanged()
            }, this, L.extend({resultFn: this.options.geocoder.geocode, resultContext: this.options.geocoder, autocompleteFn: this.options.geocoder.suggest, autocompleteContext: this.options.geocoder}, this.options.autocompleteOptions));
            return geocoderElem
        }, _updateGeocoders: function (e) {
            var newElems = [], i, geocoderElem, beforeElem;
            ...
            var wp = this._waypoints[i], value = wp && wp.name ? wp.name : "";
            ...
            for (i = 0; i < this._waypoints.length; i++) {
                if (this._waypoints[i].latLng) {
                    icon = typeof this.options.waypointIcon === "function" ? this.options.waypointIcon(i, this._waypoints[i].name, this._waypoints.length) : this.options.waypointIcon;
            ...
            if (arguments.length >= 2) {
                this.fire("waypointsspliced", {index: Array.prototype.shift.call(arguments), nRemoved: Array.prototype.shift.call(arguments), added: arguments})
            }
            ...
            var i;
            this._newWp = {afterIndex: e.afterIndex, marker: L.marker(e.latlng).addTo(this._map), lines: []};
            for (i = 0; i < this.options.dragStyles.length; i++) {
                this._newWp.lines.push(L.polyline([this._waypoints[e.afterIndex].latLng, e.latlng, this._waypoints[e.afterIndex + 1].latLng], this.options.dragStyles[i]).addTo(this._map))
            }
    ...
    (function () {
        "use strict";
        L.Routing.Control = L.Routing.Itinerary.extend({options: {fitSelectedRoutes: true}, initialize: function (options) {
            L.Util.setOptions(this, options);
